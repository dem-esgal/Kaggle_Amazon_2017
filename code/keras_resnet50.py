...
if predict:
    ...
    while (batch_id*batch_size) < len(test_df):
        for i in range(batch_size):
            ...
            if pos>=len(test_df): break
            name = train_df.image_name[pos]
            # read image
            img = img_path + name + ".jpg"
            img = cv2.imread(img)
            img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        ...
        for i in range( len(weather_vec) ):
            tags = []
            tags.append( weather_labels[weather_vec[i].argmax()] )
            for idx,aProb in enumerate(other_vec[i]):
                if aProb > threshold: tags.append( other_labels[idx] )
            tags_col.append( tags.join(" ") )
