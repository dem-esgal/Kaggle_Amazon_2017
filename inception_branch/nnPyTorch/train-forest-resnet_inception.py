...
def get_model(init_file=None):
    ...
    net_Inception = Net_Inception(
        in_shape=(1, SIZE, SIZE), num_classes=num_classes)
        ...
        if isinstance(init_content, dict):
            if 'epoch' in init_content:
                ...
                skip_list = ['Conv2d_1a_3x3.conv.weight',
                             'fc.weight', 'fc.bias']
                load_model_weight(
                    net_Inception, init_content_inception, skip_list=skip_list)
                tmp = torch.mean(init_content_inception[
                                 'Conv2d_1a_3x3.conv.weight'], 1)
                if isinstance(tmp, torch.nn.Parameter):
                    tmp = tmp.data
                net_Inception.state_dict()['Conv2d_1a_3x3.conv.weight'][
                    :, 0, :, :] = tmp
    ...
    for epoch in range(start_epoch, num_epoches):  # loop over the dataset multiple times
        ...
        if epoch % epoch_test == epoch_test - 1 or epoch == num_epoches - 1:
            ...
            if test_acc > best_acc:
                best_acc = test_acc
                torch.save(net, out_dir + '/snap/best_acc_%s_%03d.torch' %
                           (("%.4f" % best_acc).replace('.', 'd'), epoch + 1))
                torch.save(net_Inception, out_dir + '/snap/best_acc_inception_%s_%03d.torch' %
                           (("%.4f" % best_acc).replace('.', 'd'), epoch + 1))
                # modified by steve
                torch.save(fc_cat, out_dir + '/snap/best_acc_fc_cat_%s_%03d.torch' %
                           (("%.4f" % best_acc).replace('.', 'd'), epoch + 1))
                # modified by steve
                torch.save(fc_v2, out_dir + '/snap/best_acc_fc_v2_%s_%03d.torch' %
                           (("%.4f" % best_acc).replace('.', 'd'), epoch + 1))
